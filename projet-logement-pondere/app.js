...
function initAuth() {
  ...
  els.loginForm.addEventListener('submit', (event) => {
    ...
    if (!email || !password) {
      els.loginMessage.textContent = 'Veuillez remplir l\'email et le mot de passe.';
      return;
    }
    
    const users = safeParse(window.localStorage.getItem(USERS_KEY), []);
    const accountsArray = Array.isArray(users) ? users : [];
    
    let match = accountsArray.find((user) => String(user.email).toLowerCase() === email && user.password === password);
    
    if (!match) {
      // Créer automatiquement le compte pour cet email/mot de passe
      match = { email, password };
      accountsArray.push(match);
      window.localStorage.setItem(USERS_KEY, JSON.stringify(accountsArray));
    }
